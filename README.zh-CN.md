...
- **实时桌面效果**：透过绕底部铰链旋转的虚拟玻璃投影桌面，远端随距离渐进失焦、变暗；角度停止后保持姿态，桌面内容持续更新。
...
- 当前仅处理一块屏幕，上限 2560 像素宽、30 fps。渲染使用 Core Image / Metal 铰链透视投影、距离驱动的模糊与深色柔边，效果输出上限 1920 像素宽；屏幕采集使用 ScreenCaptureKit 并排除自身进程，避免递归捕获。尚未做多显示器、HDR 色彩或高刷新率优化。
...
桌面透过绕底部铰链旋转的虚拟玻璃呈现。减小角度时上部向内收窄、逐渐失焦并变暗，底部保持清晰；增大角度超过基准后反向投影。角度停下后保持姿态。基准角度处隐藏覆盖层，恢复正常桌面操作。

通过铰链玻璃光学模型，从固定观察点穿过旋转玻璃，将每个像素投影回桌面平面。双向开合时底部铰链都保持固定，上部内容随透视自然移动，最大虚拟倾角为 50°。清晰、轻模糊和重模糊纹理在同一投影坐标上混合，模糊和衰减随玻璃离开桌面的距离增加，轮廓柔和过渡到黑色。模糊半径按图像高度缩放，使不同分辨率下的观感一致。GPU 渲染在后台执行，同时最多处理一帧，后续输入只保留最新画面与角度；回到基准立即显示原图。
