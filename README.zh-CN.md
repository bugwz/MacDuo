...
- **实时桌面效果**：整块桌面绕屏幕底部铰链旋转，角度停止后保持姿态，桌面内容持续更新。
...
- 当前仅处理一块屏幕，上限 2560 像素宽、30 fps。渲染使用 Core Animation 整屏透视旋转、浅色模糊背景与边缘渐隐；屏幕采集使用 ScreenCaptureKit 并排除自身进程，避免递归捕获。尚未做多显示器、HDR 色彩或高刷新率优化。
...
完整桌面绕底部横向铰链旋转。减小角度时合下，回到基准时恢复完整桌面，继续增大角度时向另一方向倾斜；角度停下后保持姿态。基准角度处隐藏覆盖层，恢复正常桌面操作。

最大视觉倾角为 28°，周围使用低分辨率异步高斯模糊的浅色实时桌面背景，屏幕边缘随开合幅度渐隐。旋转屏幕与背景分层合成；基准附近显示原图，避免白色遮罩挡住桌面。
